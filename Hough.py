...
class Hough(object):
    ...
    def __init__(self,image, n_theta, n_rho, plot_point):
        ...
        self.n_rho= n_rho
        thetas = np.deg2rad(np.linspace(-90.0, 90.0,self.n_theta))
        width, height = self.image.shape
        diag_len = np.ceil(np.sqrt(width * width + height * height))   # max_dist
        rhos = np.linspace(-diag_len, diag_len, self.n_rho+1)#per fare nrho intervalli
        # Cache some resuable values
        cos_t = np.cos(thetas)
        sin_t = np.sin(thetas)
        num_thetas = len(thetas)
        # Hough accumulator array of theta vs rho
        accumulator = np.zeros((self.n_rho, num_thetas), dtype=np.uint64)
        y_idxs, x_idxs = np.nonzero(self.image)  # (row, col) indexes to edges
       # Vote in the hough accumulator
        for i in range(len(x_idxs)):
           ...
           for t_idx in range(num_thetas):
               ...
               ind_rho = Hough.__binary_int__(rhos,rho)
               accumulator[ind_rho, t_idx] += 1
               if plot_point is not None:
                   plot_point(rho, np.rad2deg(thetas[t_idx%accumulator.shape[1]]), accumulator[ind_rho, t_idx], step)
                   step+=1
